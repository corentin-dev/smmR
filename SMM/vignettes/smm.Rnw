...
\begin{itemize}
  ...
  \item {\tt seq}: A List of vectors representing the sequence(s) from which to estimate.
...
\begin{lstlisting}
  > est2 <- fitsemimarkovmodel(seq = seq2, E = E, type.sojourn = "fj")
\end{lstlisting}
...
\item Attribute {\tt laws}: Array of size $S \times S \times Kmax$ with estimated 
...
\begin{itemize}
  ...
  \item {\tt seq}: A list of vectors representing the sequences for which the 
...
\begin{lstlisting}
  ...
  > 
  > # Model specification
  > smmspec <- smmparametric(E = E, init = vect.init, ptrans = pij, 
  ...
  > est <- fitsemimarkovmodel(seq = seq5000, E = E, 
  +                           type.sojourn = "f", distr = "pois")
  > 
  > # Computation of  the log-likelihood
  > loglik(x = est, seq = seq5000, E = E)
  [1] -1746.548
\end{lstlisting}
...
\begin{lstlisting}
  > loglik(x = est, seq = seq1, E = E)
  [1]  -1384.444 -13833.127  -2769.192
\end{lstlisting}
...
\begin{lstlisting}
  > # Computation of the AIC
  > aic(x = est, seq = seq1, E = E)
  [1]  2864.889 27762.254  5634.383
\end{lstlisting}
