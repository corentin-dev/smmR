...
uniform, geometric, poisson, discrete weibull and binomial negative.   
}
...
$f_{14}(\cdot)$ is a negative binomial with parameters $4$ and $2$. In other words, the
parameters of $f_{13}(\cdot)$ are given in the vector {\tt param.array[1, 3, ]} that is
equal to {\tt (0.4, NA)} and the parameters of $f_{14}(\cdot)$ are given in the vector
{\tt param.array[1, 4, ]} that is equal to {\tt (4, 2)}; that means that if a distribution
has only $1$ parameter, the corresponding vector of parameters will have {\tt NA} on 
the second position.
...
\begin{lstlisting}
  ...
  > seq1[[1]][1:15]
  [1] "g" "g" "g" "g" "c" "c" "t" "g" "g" "a" "a" "a" "t" "g" "g"
\end{lstlisting}
...
\begin{itemize}
  ...
  \item Attribute {\tt init}: Vector of size $S$ with the estimated initial 
    ...
    \begin{lstlisting}
      > est1$init
      [1] 0.1931263 0.2581568 0.2753738 0.2733431
    \end{lstlisting}
  ...
  \item Attribute {\tt ptrans}: Matrix of size  $S\times S$ with the estimated 
    ...
    \begin{lstlisting}
      > est1$ptrans
                [,1]      [,2]      [,3]      [,4]
      [1,] 0.0000000 0.2079208 0.4884488 0.3036304
      [2,] 0.2042553 0.0000000 0.3021277 0.4936170
      [3,] 0.2959002 0.4973262 0.0000000 0.2067736
      [4,] 0.3958763 0.1989691 0.4051546 0.0000000
    \end{lstlisting}
...
\item Attribute {\tt param}: Array with the estimated parameters of the sojourn 
  ...
  \begin{lstlisting}
    ...
    , , 1
              ...
              [,1]      [,2]      [,3]      [,4]
    [1,]        NA 1.8941799 0.4084637 5.6396067
    [2,] 0.6760563        NA 5.1549296 0.5457669
    [3,] 1.9096386 2.9820789        NA 0.6025974
    [4,] 4.0859375 0.2915408 0.4248649        NA
    
    , , 2
    
         [,1] [,2] [,3]      [,4]
    [1,]   NA   NA   NA 2.1593694
    [2,]   NA   NA   NA 0.7557712
    [3,]   NA   NA   NA        NA
    [4,]   NA   NA   NA        NA
  \end{lstlisting}
\end{itemize}
