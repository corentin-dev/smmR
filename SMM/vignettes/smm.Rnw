...
\begin{itemize}
  ...
  \item[$\blacksquare$] \fct{fitsmm} for the estimation of model parameters
    (Section~\ref{estSM}):
    {
    \begin{itemize}
      \item One or several trajectories;
...
\begin{itemize}
  \item[$\blacksquare$] \fct{mm} for the specification of a $k$th 
    order Markov model specification (object of class S3);
    
  \item[$\blacksquare$] \fct{simulate.mm} A method of S3 class 
    \texttt{mm} for the simulation of sequences from a $k$th order 
    Markov model;
    
  \item[$\blacksquare$] \fct{fitmm} for the estimation of the 
...
The estimation is carried out by the function \fct{fitsmm}. The 
...
estparam <- fitsmm(sequences = seqparam, states = states, 
                               type.sojourn = "fij", distr = distr.matrix)
@
...
The function \fct{fitsmm} returns an object of class \class{smm}, 
...
<<estnonparam>>=
estnonparam <- fitsmm(sequences = seqnonparam, states = states, 
                                  type.sojourn = "fj")
@

Here, we estimate simulated sequences with no censoring.

The function \fct{fitsmm} returns an object of class \class{smm}, more
...
est <- fitsmm(sequences = seq5000, states = states, 
...
\begin{itemize}
  \item[$\blacksquare$] \fct{mm}: Specification of a Markov chain of order $k$
\end{itemize}
...
markovspec <- mm(states = states, init = vect.init, ptrans = p, k = k)
@

\paragraph{Values:}
The function \fct{mm} returns an object of class \class{mm}. The S3 
class \class{mm} contains various methods such as \fct{simulate}.
...
\begin{itemize}
\item[$\blacksquare$] \fct{simulate.mm}: Simulation of Markov chains of order $k$
\end{itemize}
...
\begin{itemize}
  \item[$\blacksquare$] \fct{fitmm}: Estimation of a Markov chain of order $k$
\end{itemize}

<<estmarkov>>=
# State space
states <- c("a", "c", "g", "t")
estmarkov <- fitmm(sequences = seqmarkov, states = states, k = 2)
@

\paragraph{Values:}

The function \fct{fitmm} returns an object of class \class{mm}:
...
\begin{itemize}
  ...
  \item \code{x}: An object of the class \class{mm}.
...
\begin{itemize}
  ...
  \item \code{x}: An object of class {\tt mm}.
