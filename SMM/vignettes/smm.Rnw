...
% D\'efinition des couleurs utilis\'ees dans le document (Titres + Listings) %
...
% \author{
% Vlad Stefan Barbu \And Caroline B\'erard \And Dominique Cellier \And Florian Lecocq \And  Corentin Lothod\'e \And Mathilde Sautreuil \AND Nicolas Vergne \\ Universit\'e de Rouen--Normandie
% }

\author{
Vlad Stefan Barbu \And Caroline B\'erard \And Dominique Cellier
\AND Florian Lecocq \And Corentin Lothod\'e \And Mathilde Sautreuil\\
\AND Nicolas Vergne \\ Universit\'e de Rouen--Normandie
}
...
% \Plainauthor{Vlad Stefan Barbu, Caroline B\'erard, Dominique Cellier, Florian Lecocq, \\ Corentin Lothod\'e, Mathilde Sautreuil, Nicolas Vergne} %% comma-separated
...
of discrete-time multi-state semi-Markov and Markov models. 
For the semi-Markov case we have considered: parametric and non-parametric estimation;
with and without censoring at the beginning and/or at the end of sample paths; one or
several independent sample paths. Several discrete-time distributions are considered 
for the parametric estimation of sojourn time distributions of semi-Markov chains: 
...
\begin{itemize}
\item Parametric estimation for sojourn time distributions (uniform, geometric, poisson, discrete weibull and negative binomial) or non-parametric estimation;
...
The \pkg{SMM} \proglang{R} package is principally devoted to the simulation and estimation of discrete-time semi-Markov models in different cases by the following functions:

\begin{itemize}
\item[$\blacksquare$] \texttt{smmparametric} : A constructor for the creation of 
  a parametric Semi-Markov model specification (object of class S3);
  
\item[$\blacksquare$] \texttt{smmnonparametric} : A constructor for the creation of a 
  non-parametric Semi-Markov model specification (object of class S3);
  
\item[$\blacksquare$] \texttt{simulate.smmparametric} and 
  \texttt{simulate.smmnonparametric} : Methods of S3 classes \texttt{smmparametric} 
    and \texttt{smmnonparametric} for the simulation of sequences from a parametric 
...
\item[$\blacksquare$] \texttt{fitsemimarkovmodel} for the estimation of model parameters
  (Section~\ref{estSM}):
  {
  \begin{itemize}
    \item One or several trajectories;
    \item Parametric (uniform, geometric, poisson, discrete weibull and negative binomial)
      or non-parametric distributions for the sojourn times;
    \item Four different types of censoring mechanisms: censoring at the beginning of
...
The \pkg{SMM} \proglang{R} package is also devoted to the simulation and estimation of discrete-time Markov models by the following functions:
\begin{itemize}
  \item[$\blacksquare$] \texttt{markovmodel} for the specification of a $k$th order 
    Markov model specification (object of class S3);
    
  \item[$\blacksquare$] \texttt{simulate.markovmodel} A method of S3 class 
    \texttt{markovmodel} for the simulation of sequences from a $k$th order Markov model;
    
  \item[$\blacksquare$] \texttt{fitmarkovmodel} for the estimation of the parameters 
  of a Markov model.
\end{itemize}
...
\subsection{Semi-Markov Model specifications} \label{sec_SpecSM}

\paragraph{3.1.1 Specification according to classical distributions - smmparametric}
\ \\

In this part, we will consider parametric specification that is based on classical distributions.

\paragraph{Parameters:} This specification is carried out by the constructor function {\tt smmparametric}. The different parameters of the function are:
%
\begin{itemize}
  \item {\tt states}: Vector of state space of length $s$.
  
  \item {\tt init}: Vector of initial distribution of length $s$.
  
  \item {\tt ptrans}:  Matrix of transition probabilities of the embedded Markov 
    chain $J=(J_m)_{m}$ of size  $s\times s$.
    
  \item {\tt type.sojourn}: Type of sojourn time; it can be {\tt "fij"}, {\tt "fi"}, 
    {\tt "fj"} or {\tt "f"} according to the four cases previously discussed.

  \item {\tt distr}: Sojourn time distributions:
    {
    \begin{itemize}
      \item  Matrix of distributions of size $s\times s$ if {\tt type.sojourn = "fij"};
      \item Vector of distributions of size $s$ if  {\tt type.sojourn = "fi"} or 
        {\tt type.sojourn = "fj"};
      \item A distribution if {\tt type.sojourn = "f"}.
    \end{itemize}
    }
    where the distributions to be used can be one of {\tt "unif"}, {\tt "geom"}, 
    {\tt "pois"}, {\tt "dweibull"} or {\tt "nbinom"}.
  ...
  \item {\tt param}: Parameters of sojourn time distributions: 
    ...
    \begin{itemize}
      \item  An array of parameters of size $s\times s \times 2$ if 
        {\tt type.sojourn = "fij"};
      \item A matrix of parameters of size $s \times 2$ if {\tt type.sojourn = "fi"}
        or {\tt type.sojourn = "fj"};
      \item A vector of parameters of length $2$ if {\tt type.sojourn = "f"}.
    \end{itemize}
    }
    When parameters/values are not necessary (e.g. the Poisson distribution has 
    only one parameter that is $\lambda$, leave the value {\tt NA} for the 
    second parameter in the argument {\tt param}).

  \item {\tt cens.beg}: Optional. Type of censoring at the beginning of sample paths; 
    {\tt TRUE} (if the first sojourn time is censored) or {\tt FALSE} (if not). 
    All the sequences must be censored in the same way.

  \item {\tt cens.end}: Optional. Type of censoring at the end of sample paths; 
    {\tt TRUE} (if the last sojourn time is censored) or {\tt FALSE} (if not). 
    All the sequences must be censored in the same way.

\end{itemize}


The \proglang{R} commands below creates a parametric semi-Markov model specification 
(i.e. an object of class {\tt smmparametric}) with the finite state space 
$E = \{a, c, g, t\}$, where the sojourn times depend on the current state and on 
the next state ($f_{ij}$).
...
\end{lstlisting}
%
First, note that in this simulation, the parameters {\tt cens.beg} and {\tt cens.end} 
are equal to {\tt FALSE}, that is to say assumption is made that there is no censoring,
at the beginning and at the end.

Second, note also that the parameters of the distributions are given in the following way:
for example, $f_{13}(\cdot)$ is a geometric distribution with parameter $p = 0.4$, while
$f_{14}(\cdot)$ is a negative binomial with parameters $\mu = 4$ and $p = 0.6$. 
In other words, the parameters of $f_{13}(\cdot)$ are given in the vector 
{\tt param.array[1, 3, ]} that is equal to {\tt (0.4, NA)} and the parameters of 
$f_{14}(\cdot)$ are given in the vector {\tt param.array[1, 4, ]} that is equal to 
{\tt (4, 2)}; that means that if a distribution has only $1$ parameter, the 
corresponding vector of parameters will have {\tt NA} on the second position.

\paragraph{Values:}

The function {\tt smmparametric} returns an object of class {\tt smmparametric}. The S3 
class {\tt smmparametric} contains various methods such as {\tt simulate}.
...
\begin{lstlisting}
  ...
  $distr
       [,1]   [,2]   [,3]   [,4]      
  [1,] NA     "pois" "geom" "nbinom"  
  [2,] "geom" NA     "pois" "dweibull"
  [3,] "pois" "pois" NA     "geom"    
  [4,] "pois" "geom" "geom" NA        
...
\paragraph{3.1.2 Specification according to distributions given by the user - smmnonparametric}
\ \\ 

Now we will specify a non-parametric semi-Markov model (i.e. the distributions are given 
by the user).

\paragraph{Parameters:} Specifying the model is done by the function 
{\tt smmnonparametric}. The different parameters of the function are:
...
\begin{itemize}
  ...
  \item {\tt distr}: Sojourn time distributions introduced by the user: 
    {
    \begin{itemize}
      \item  An array of size $s\times s \times kmax$ if {\tt type.sojourn = "fij"};
      \item A matrix of size $s \times kmax$ if {\tt type.sojourn = "fi"} or 
        {\tt type.sojourn = "fj"};
      \item A vector of length $kmax$ if {\tt type.sojourn = "f"}.
    \end{itemize}
...
specification (i.e. an object of class {\tt smmnonparametric}) with the finite 
...
\paragraph{Values:}
The function {\tt smmnonparametric} returns an object of class {\tt smmnonparametric}. 
The S3 class {\tt smmnonparametric} contains various methods such as {\tt simulate}.
...
\begin{lstlisting}
  ...
  $distr
       [,1] [,2] [,3] [,4] [,5] [,6]
  [1,]  0.2  0.1 0.30 0.20  0.2  0.0
  [2,]  0.4  0.2 0.10 0.00  0.2  0.1
  [3,]  0.5  0.3 0.15 0.05  0.0  0.0
  [4,]  0.3  0.2 0.10 0.20  0.2  0.0
...
To simulate some sequences based from a semi-Markov model, the method {\tt simulate}
has been implemented.

\paragraph{3.2.1 Simulation according to classical distributions - smmparametric}\ \\

Let's use the object {\tt smm1} to simulate $3$ sequences of size $1,000$, $10,000$ 
and $2,000$ respectively with the finite state space $E = \{a, c, g, t\}$:

\begin{lstlisting}
  > seq1 <- simulate(object = smm1, nsim = c(1000, 10000, 2000), seed = 100)
  > seq1[[1]][1:15]
   [1] "g" "g" "g" "g" "c" "c" "t" "g" "g" "a" "a" "a" "a" "t" "g"
\end{lstlisting}

\paragraph{3.2.2 Simulation according to a distribution given by the user - smmnonparametric}\ \\

Let's use the object {\tt smm2} to simulate $3$ sequences of size $1,000$, $10,000$ 
and $2,000$ respectively with the finite state space $E = \{a, c, g, t\}$:

\begin{lstlisting}
  > seq2 <- simulate(object = smm2, nsim = c(1000, 10000, 2000), seed = 100)
  > seq2[[1]][1:15]
   [1] "g" "g" "g" "g" "g" "c" "c" "t" "t" "t" "g" "g" "g" "g" "g"
\end{lstlisting}
...
In this subsection we explain and illustrate the estimation of a semi-Markov model 
in the parametric case and non-parametric case.

\paragraph{3.3.1 Parametric estimation of semi-Markov models}
\ \\

We will consider the distributions $f_{ij}(k) = f_{ij}(k; \theta_{ij})$ depending on
unknown parameters $\theta_{ij} \in \mathbb R^{m_{ij}},$ where the dimension of 
parameters set $m_{ij}$ is known; no assumptions is done on $\left(p_{ij}\right)_{ij}$.
From data, we want to estimate $p_{ij}$ et $\theta_{ij},$ $i,j \in E$.

Let us assume that we have at our disposal several independent sample paths of a 
semi-Markov chain, say $L$, each of them of length $n_l$, $l=1, \ldots, L$, censored 
...
with  $\sum_{i=0}^{t^l+1} k_i^l = n_l,$ where $j_0^l, \ldots, j_{t^l+1}^l$ are the
successive distinct visited states, $ k_{0}^l $ is the first sojourn time, assumed 
to be right censored, $ k_{t^l +1}^l $ is the last sojourn time, assumed also to be 
right censored, while $k_1^l, \ldots, k_{t^l}^l$ are the other successive sojourn times,
assumed to be complete (observed, non censored).
...
- $\overline{N}_{ij}^b(k; L)$ represents the number of trajectories starting in state 
$i,$ with a next transition to state $j$ and censored sojourn time in state $i$ 
greater than $k$; \\

- $\overline{N}_{i \bullet}^e(k; L)$ represents the number of trajectories ending in 
state $i$ with a censored sojourn time in state $i$ greater than $k.$\\

Note also that in the expression \eqref{EMV} of the log-likelihood, the first two terms
correspond to the transition probabilities and the observed (non censored) sojourn times,
the third term is the contribution to the likelihood of the first sojourn times, assumed
to be right censored, while the last term is the contribution to the likelihood of the
last sojourn times, assumed to be right censored.\\

Up to here we presented the estimation for the general case, that is to say taking into
account the censoring at the beginning and at the end and the sojourn times 
depending on the current state and on the next state.  Thus the maximization problem
\eqref{EMV} is written with the sojourn times depending on the current state and on the
next state (the general model of the type $q_{ij}(k)=p_{ij}f_{ij}(k)$ given in 
\eqref{eq_kernel_fij}), but the different cases of sojourn times are written and coded 
in the package. Note also that different types of censoring are also written and coded 
in the package. %For more details, see \citep{BaBeCeSaVe}.

\paragraph{Parameters:}
The estimation is carried out by the function {\tt fitsemimarkovmodel}. The different
parameters of the function are:

\begin{itemize}

  \item {\tt sequences}: A List of vectors representing the sequence(s) from which to estimate.
  ...
  \item {\tt distr}: By default {\tt "nonparametric"} for the non-parametric estimation 
    case. If the parametric estimation case is desired, {\tt distr} should be:
    {
    \begin{itemize}
      \item  Matrix of distributions of size $s\times s$ if {\tt type.sojourn = "fij"};
      \item Vector of distributions of size $s$ if  {\tt type.sojourn = "fi"} or 
...
Here, we estimate simulated sequences with no censoring. The estimation performed will
correspond to the likelihood given in \eqref{EMV}, without the third and fourth terms. 
For more details on the parametric estimation, one can see \cite{BaBeCeSaVe}.

\paragraph{Values:}

The function {\tt fitsemimarkovmodel} returns an object of class {\tt smm}, more
precisely an object of class {\tt smmparametric} if a parametric estimation is required 
or an object of class {\tt smmnonparametric} otherwise:
...
\begin{itemize}
  ...
  \item Attribute {\tt init}: Vector of size $s$ with the estimated initial 
    probabilities of the semi-Markov chain
    
    \begin{lstlisting}
      > est1$init
      [1] 0.2056339 0.2527220 0.2715249 0.2701192
    \end{lstlisting}
  
  \item Attribute {\tt ptrans}: Matrix of size  $s\times s$ with the estimated 
    transition probabilities of the embedded Markov chain $J=(J_m)_{m}$

    \begin{lstlisting}
      > est1$ptrans
                [,1]      [,2]      [,3]      [,4]
      [1,] 0.0000000 0.2081949 0.4905869 0.3012182
      [2,] 0.2081985 0.0000000 0.2988134 0.4929881
      [3,] 0.2987365 0.4936823 0.0000000 0.2075812
      [4,] 0.3960293 0.1985371 0.4054336 0.0000000
    \end{lstlisting}
    
\item Attribute {\tt param}: Array with the estimated parameters of the sojourn 
  time distributions
  \begin{lstlisting}
    > est1$param
    , , 1
              ...
              [,1]      [,2]      [,3]      [,4]
    [1,]        NA 1.8776596 0.4075437 4.2673358
    [2,] 0.6819788        NA 5.1696751 0.5415246
    [3,] 1.9063444 2.9853748        NA 0.6052632
    [4,] 4.0870712 0.2883156 0.4245077        NA
    
    , , 2
    
         [,1] [,2] [,3]      [,4]
    [1,]   NA   NA   NA 0.6020990
    [2,]   NA   NA   NA 0.7488554
    [3,]   NA   NA   NA        NA
    [4,]   NA   NA   NA        NA
  \end{lstlisting}
\end{itemize}

Note that, for example, {\tt est1\$param[1, 3, ]} is the vector containing the 
...
cf.~\citet{BL06,BarbuLimnios2008} and an estimation based on a couple Markov chain
associated to the semi-Markov chain \citep[see][]{TL11}.
...
$$
where $N_{ij}(n) = \sum_{m=1}^{N(n)-1}  \mi_{\{J_m = i ; J_{m+1}=j\}}$, 
$N_{i\bullet}(n) = \sum_{m=1}^{N(n)-1} \mi_{\{J_m = i\}}$,\\
...
For a semi-Markov chain $Y=(Y_n)_{n\in\mathbb{N}}$, let $U=(U_n)_{n\in\mathbb{N}}$ be 
the backward recurrence time of the SMC, defined by 
...
In order to compute the estimators of the sojourn times, we consider the four different
types of semi-Markov kernels defined in Equations (\ref{eq_kernel_fij}), 
(\ref{eq_kernel_fi}), (\ref{eq_kernel_fj}) and (\ref{eq_kernel_f}).

\paragraph{Parameters:}
The estimation is carried out by the function {\tt fitsemimarkomodel} and several parameters must be given.
...
\begin{itemize}
  ...
  \item {\tt distr}: Let's keep the default value {\tt "nonparametric"}.
...
\begin{lstlisting}
  > # Estimation of simulated sequences
  > est2 <- fitsemimarkovmodel(sequences = seq2, states = states, type.sojourn = "fj")
\end{lstlisting}
...
\begin{itemize}
  ...
  \item Attribute {\tt init}: Vector of size $s$ with the estimated initial 
    ...
    \begin{lstlisting}
      > est2$init
      [1] 0.2078682 0.2409731 0.3101855 0.2409731
    \end{lstlisting}
  ...
  \item Attribute {\tt ptrans}: Matrix of size  $s\times s$ with the estimated 
    ...
    \begin{lstlisting}
      > est2$ptrans
                [,1]      [,2]      [,3]      [,4]
      [1,] 0.0000000 0.2012038 0.4772141 0.3215821
      [2,] 0.1980519 0.0000000 0.2913961 0.5105519
      [3,] 0.2930200 0.5093296 0.0000000 0.1976503
      [4,] 0.3840186 0.2024825 0.4134988 0.0000000
    \end{lstlisting}
    
\item Attribute {\tt distr}: Array of size $s \times s \times kmax$ with estimated 
  values of the sojourn time distributions
  \begin{lstlisting}
    > est2$distr
              [,1]       [,2]       [,3]       [,4]      [,5]       [,6]
    [1,] 0.2132416 0.08856406 0.28890800 0.21582115 0.1934652 0.00000000
    [2,] 0.4107143 0.19480519 0.09821429 0.00000000 0.1980519 0.09821429
    [3,] 0.4968901 0.29371113 0.14858328 0.06081548 0.0000000 0.00000000
    [4,] 0.3087665 0.20791311 0.09619860 0.20480993 0.1823119 0.00000000
  \end{lstlisting}
\end{itemize}
...
In this package, others functions are available. These functions enable to compute the 
log-likelihood of a semi-Markov model and the AIC and BIC of a semi-Markov model.

\begin{itemize}
  \item[$\blacksquare$] {\tt loglik}: Computation of the log-likelihood
\end{itemize}
...
\begin{itemize}
  ...
  \item {\tt x}: An object inheriting from the class {\tt smm}
    ({\tt smmnonparametric} or {\tt smmnparametric}).
    
  \item {\tt sequences}: A list of vectors representing the sequences for which the 
...
\begin{lstlisting}
  ...
  +                          type.sojourn = "f", distr = "pois", param = 2)
  ...
  > loglik(x = est, sequences = seq5000, states = states)
  [1] -4594.97
\end{lstlisting}

\paragraph{Values:}

The function {\tt loglik} returns a vector giving the value of the loglikelihood 
for each sequence.

We also consider model selection criteria in order to evaluate and choose among 
candidate models; the Akaike information criterion (AIC) and the Bayesian information
criterion (BIC) are considered.

\begin{itemize}
\item[$\blacksquare$] {\tt aic}: computation of the AIC
\end{itemize}
%
...
\begin{lstlisting}
  > aic(x = est, sequences = seq5000, states = states)
  [1] 9191.939
\end{lstlisting}

\paragraph{Values:}

The function {\tt aic} returns vector giving the value of the AIC for each sequence.

\begin{itemize}
  \item[$\blacksquare$] {\tt bic}: computation of the BIC
\end{itemize}
...
\begin{lstlisting}
  > bic(x = est, sequences = seq5000, states = states)
  [1] 9198.457
\end{lstlisting}

\paragraph{Values:}

The function {\tt bic} returns vector giving the value of the BIC for each sequence.



\subsection{Markov case}

In the {\tt SMM R} package, we also implemented the estimation and the simulation 
of discrete-time multi-state Markov models. 
As in the semi-Markov case, others functions are available, enabling to compute the 
log-likelihood and also the AIC and BIC of a Markov model.

\begin{itemize}
  \item[$\blacksquare$] {\tt markovmodel}: Specification of a Markov chain of order $k$
\end{itemize}
...
\paragraph{Values:}
The function {\tt markovmodel} returns an object of class {\tt markovmodel}. The S3 
class {\tt markovmodel} contains various methods such as {\tt simulate}.
...
\begin{itemize}
\item[$\blacksquare$] {\tt simulate.markovmodel}: Simulation of Markov chains of order $k$
\end{itemize}

Let's use the object {\tt markov1} to simulate 3 sequences of length $1000$, $10000$ and
$2000$.

\begin{lstlisting}
  > seq1 <- simulate(object = markov1, nsim = c(1000, 10000, 2000), seed = 150)
  > seq1[[1]][1:15]
   [1] "c" "g" "g" "a" "c" "g" "g" "g" "g" "a" "t" "a" "a" "g" "t"
\end{lstlisting}

\begin{itemize}
  \item[$\blacksquare$] {\tt fitmarkovmodel}: Estimation of a Markov chain of order $k$
\end{itemize}
...
The function {\tt fitmarkovmodel} returns an object of class {\tt markovmodel}:
...
\begin{itemize}
  ...
  \item {\tt x}: An object of the class {\tt markovmodel}.
...
\begin{lstlisting}
  > # Computation of  the log-likelihood
  > loglik(x = est, sequences = seq1, states = states)
  [1] -17986.76
\end{lstlisting}
...
\begin{itemize}
  \item[$\blacksquare$] {\tt aic}: Computation of the AIC for a Markov chain of order $k$
\end{itemize}
...
\begin{itemize}
  ...
  \item {\tt x}: An object of class {\tt markovmodel}.
...
\begin{lstlisting}
  ...
  > aic(x = est, sequences = seq1, states = states)
  [1] 36069.53
\end{lstlisting}
