...
reliab <- reliability(x = factory, k = k, upstates = upstates)
```
...
```{r}
failureBMP <- failureRateBMP(x = factory, k = k, upstates = upstates)
```
...
```{r}
failureRG <- failureRateRG(x = factory, k = k, upstates = upstates)
```
