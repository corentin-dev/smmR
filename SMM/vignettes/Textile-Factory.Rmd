...
Let's introduce a practical example where a semi-Markov modeling is 
appropriate (this example is taken from the book **@BarbuLimnios2008**).
 ...
 * **State 3** - $\mathcal{U}$ has failed and $\mathcal{T}$ is full, so the 
...
```{r}
estimate <- fitsemimarkovmodel(sequences = seq, states = states, type.sojourn = "fij")
```
...
We can also plot the estimated sojourn time densities. For example, let's 
plot the sojourn time distribution $\hat{f}_{23}$:
...
compute some indicators such as the reliability, the availability, the 
maintainability and also their asymptotic confidence intervals.
...
```{r}
reliab <- reliability(x = factory, k = k, upstates = upstates, var = TRUE)

level <- 0.95
confInf <- reliab$x - sqrt(reliab$sigma2 / M) * qnorm(p = 1 - (1 - level) / 2)
confSup <- reliab$x + sqrt(reliab$sigma2 / M) * qnorm(p = 1 - (1 - level) / 2)
```

```{r}
plot(x = 0:k, y = reliab$x, type = "l", cex = 2.5, ylim = c(0, 1), 
     col = "blue", main = "Reliability", xlab = "k", ylab = "R(k)")

lines(x = confInf, lty = 4, col = "blue")
lines(x = confSup, lty = 4, col = "blue")
legend(x = "topright", 
       legend = c("True value", "95% confidence interval"), 
       col = "blue", lty = c(1, 4))
```
...
```{r}
avail <- availability(x = factory, k = k, upstates = upstates, var = TRUE)

level <- 0.95
confInf <- avail$x - sqrt(avail$sigma2 / M) * qnorm(p = 1 - (1 - level) / 2)
confSup <- avail$x + sqrt(avail$sigma2 / M) * qnorm(p = 1 - (1 - level) / 2)
```

```{r}
plot(x = 0:k, y = avail$x, type = "l", cex = 2.5, ylim = c(0.95, 1), 
     col = "blue", main = "Availability", xlab = "k", ylab = "A(k)")
...
```{r}
failureBMP <- failureRateBMP(x = factory, k = k, upstates = upstates, var = TRUE)

level <- 0.95
confInf <- failureBMP$x - sqrt(failureBMP$sigma2 / M) * qnorm(p = 1 - (1 - level) / 2)
confSup <- failureBMP$x + sqrt(failureBMP$sigma2 / M) * qnorm(p = 1 - (1 - level) / 2)
```

```{r}
plot(x = 0:k, y =  failureBMP$x, type = "l", cex = 2.5, ylim = c(0, 0.025), 
     col = "blue", main = "BMP-failure rate", xlab = "k", ylab = bquote(lambda(k)))
...
Discrete-time adapted failure rate, proposed by D. Roy and R. Gupta. 
Classification of discrete lives. _Microelectronics Reliability_, 
32(10):1459--1473, 1992. We call it the **RG-failure rate** and denote it by 
$r(k),\ k \in \mathbb{N}$.

```{r}
failureRG <- failureRateRG(x = factory, k = k, upstates = upstates, var = TRUE)

level <- 0.95
confInf <- failureRG$x - sqrt(failureRG$sigma2 / M) * qnorm(p = 1 - (1 - level) / 2)
confSup <- failureRG$x + sqrt(failureRG$sigma2 / M) * qnorm(p = 1 - (1 - level) / 2)
```

```{r}
plot(x = 0:k, y =  failureRG$x, type = "l", cex = 2.5, ylim = c(0, 0.025), 
     col = "blue", main = "RG-failure rate", xlab = "k", ylab = "r(k)")
...
```{r}
mttf <- mttf(x = factory, upstates = upstates, var = TRUE)

level <- 0.95
confInf <- mttf$x - sqrt(mttf$sigma2 / M) * qnorm(p = 1 - (1 - level) / 2)
confSup <- mttf$x + sqrt(mttf$sigma2 / M) * qnorm(p = 1 - (1 - level) / 2)
```

```{r}
print(rbind(confInf, mttf$x, confSup))
```
...
```{r}
mttr(x = factory, downstates = downstates, var = TRUE)
```
