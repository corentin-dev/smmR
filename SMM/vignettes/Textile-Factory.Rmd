...
level <- 0.95
confInf <- reliab$reliab - sqrt(reliab$sigma2 / M) * qnorm(p = 1 - (1 - level) / 2)
confSup <- reliab$reliab + sqrt(reliab$sigma2 / M) * qnorm(p = 1 - (1 - level) / 2)
```

```{r}
plot(x = 0:k, y = reliab$reliab, type = "l", cex = 2.5, ylim = c(0, 1), 
...
level <- 0.95
confInf <- avail$avail - sqrt(avail$sigma2 / M) * qnorm(p = 1 - (1 - level) / 2)
confSup <- avail$avail + sqrt(avail$sigma2 / M) * qnorm(p = 1 - (1 - level) / 2)
```

```{r}
plot(x = 0:k, y = avail$avail, type = "l", cex = 2.5, ylim = c(0.95, 1), 
...
<!-- ```{r} -->
<!-- downstates <- c("3") # Failure state of the semi-Markov system -->
<!-- maintain <- maintainability(x = factory, k = 300, downstates = c("3")) -->
<!-- ``` -->
...
level <- 0.95
confInf <- failureBMP$lbda - sqrt(failureBMP$sigma2 / M) * qnorm(p = 1 - (1 - level) / 2)
confSup <- failureBMP$lbda + sqrt(failureBMP$sigma2 / M) * qnorm(p = 1 - (1 - level) / 2)
```

```{r}
plot(x = 0:k, y =  failureBMP$lbda, type = "l", cex = 2.5, ylim = c(0, 0.025), 
...
level <- 0.95
confInf <- failureRG$r - sqrt(failureRG$sigma2 / M) * qnorm(p = 1 - (1 - level) / 2)
confSup <- failureRG$r + sqrt(failureRG$sigma2 / M) * qnorm(p = 1 - (1 - level) / 2)
```

```{r}
plot(x = 0:k, y =  failureRG$r, type = "l", cex = 2.5, ylim = c(0, 0.025), 
...
level <- 0.95
confInf <- mttf$mttf - sqrt(mttf$sigma2 / M) * qnorm(p = 1 - (1 - level) / 2)
confSup <- mttf$mttf + sqrt(mttf$sigma2 / M) * qnorm(p = 1 - (1 - level) / 2)
```

```{r}
plot(x = 1:2, y = mttf$mttf, pch = "+", col = "red", axes = FALSE, 
     xlab = "", ylab = "", xlim = c(0.5, 2.5), ylim = c(61, 72))
axis(side = 2)
axis(side = 1, at = c(1, 2), labels = c("state 1", "state 2"), lty = 0)
segments(x0 = 1:2, y0 = mttf$mttf, y1 = confInf, lty = 4, col = "blue")
segments(x0 = 1:2, y0 = mttf$mttf, y1 = confSup, lty = 4, col = "blue")
legend(x = "topright", 
       legend = c("True value", "95% confidence interval"), 
       col = c("red", "blue"), lty = c(0, 4), pch = c("+", ""))
```
...
```{r}
mttr(x = factory, downstates = downstates)
```