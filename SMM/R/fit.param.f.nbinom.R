.fit.param.f.nbinom <- function(res, Kmax, cens.beg, cens.end) {
  ...
  loglik <- function(par) {
    return(
      -(sum(res$Nk * dnbinom(x = 0:(Kmax - 1), size = par[1], mu = par[2], log = TRUE)))
    )
  }
  ...
  if (!cens.beg && cens.end) {# Censoring at the end
    ...
    loglik <- function(par) {
      return(
        -(sum(res$Nk * dnbinom(x = 0:(Kmax - 1), size = par[1], mu = par[2], log = TRUE)) 
          + sum(res$Nek * log(pnbinom(q = 0:(Kmax - 1), size = par[1], mu = par[2], lower.tail = FALSE))))
        )
    }
    ...
    loglik <- function(par) {
      return(
        -(sum(res$Nk * dnbinom(x = 0:(Kmax - 1), size = par[1], mu = par[2], log = TRUE)) 
          + sum(res$Nbk * log(pnbinom(q = 0:(Kmax - 1), size = par[1], mu = par[2], lower.tail = FALSE))))
        )
    }
    ...
    loglik <- function(par) {
      return(
        -(sum(res$Nk * dnbinom(x = 0:(Kmax - 1), size = par[1], mu = par[2], log = TRUE)) 
          + sum(res$Nebk * log(pnbinom(q = 0:(Kmax - 1), size = par[1], mu = par[2], lower.tail = FALSE))))
        )
    }
